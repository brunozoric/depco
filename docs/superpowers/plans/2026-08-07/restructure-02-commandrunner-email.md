...
| `src/api/services/__tests__/ConsoleEmailService.test.ts:3` | `from "../abstractions/EmailService.js"` | `from "../abstractions/EmailService.js"` → `from "../Email/index.js"` (test moved into Email/__tests__/, so path changes to `"../index.js"` from within `Email/__tests__/`) |
