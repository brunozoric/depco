...
import { GetUpgradeSessionUseCase } from "../abstractions/GetUpgradeSessionUseCase.js";
...
interface ITestContext {
    container: Container;
    useCase: GetUpgradeSessionUseCase.Interface;
}
...
function createContext(options: ICreateContextOptions = {}): ITestContext {
    ...
    return { container, useCase: container.resolve(GetUpgradeSessionUseCase) };
}
...
describe("GetUpgradeSessionUseCase", () => {
    it("returns the session when it exists", async () => {
        const fixture = createSessionFixture();
        const getSession = vi.fn(async () => fixture);
        const { useCase } = createContext({ upgradeSessionService: { getSession } });
        ...
        expect(getSession).toHaveBeenCalledWith("session-1", "project-1");
    });

    it("fails with 404 when the service returns no session", async () => {
        const { useCase } = createContext({
            upgradeSessionService: { getSession: vi.fn(async () => null) }
        });
