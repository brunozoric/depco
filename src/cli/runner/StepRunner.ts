    ...
    public async run(args: Abstraction.Args): Promise<void> {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            try {
                ...
                this.logger.info(`\x1b[32m✓ ${label}\x1b[0m`);
                completed.push(step);
            } catch (error) {
                if (error instanceof StepExecutionError) {
                    throw error;
                }
                this.logger.error(`\x1b[31m✗ ${label}\x1b[0m`);
                await this.rollback(completed, context);
                throw new StepExecutionError(
                    `Step "${step.name}" threw: ${getErrorMessage(error, String(error))}`
                );
            }
        }
    }
        ...
        for (let i = completed.length - 1; i >= 0; i--) {
            ...
            if (step.rollback) {
                try {
                    ...
                    this.logger.warn(`\x1b[33m↩ Rolled back: ${step.name}\x1b[0m`);
                } catch (rollbackError) {
                    this.logger.error(
