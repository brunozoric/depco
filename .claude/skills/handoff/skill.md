...
- How many commits
- Which areas changed (src/api/, src/ui/, src/shared/, testing/)
...
| Doc         | Update if...                                         |
| ----------- | ---------------------------------------------------- |
| `AGENTS.md` | Key rules, project structure, or conventions changed |
| `CLAUDE.md` | Commands or top-level pointers changed               |
...
```
yarn lint && yarn format:check && yarn build && yarn test
```
