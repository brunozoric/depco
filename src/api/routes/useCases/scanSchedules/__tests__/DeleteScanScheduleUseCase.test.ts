...
import { DeleteScanScheduleUseCase, ScanSchedulesUseCasesFeature } from "../index.js";
...
interface ITestContext {
    ...
    useCase: DeleteScanScheduleUseCase.Interface;
...
function createContext(options: ICreateContextOptions = {}): ITestContext {
    ...
    return {
        container,
        db,
        useCase: container.resolve(DeleteScanScheduleUseCase),
        scanSchedulerService
    };
}

describe("DeleteScanScheduleUseCase", () => {
    it("deletes the project's schedule override and reschedules the project", async () => {
        ...
        expect(
            db.select().from(scanSchedules).where(eq(scanSchedules.projectId, projectId)).all()
        ).toEqual([]);
    });
    ...
    it("fails with 500 when rescheduling the project throws", async () => {
        ...
        const { useCase } = createContext({ scanSchedulerService: { scheduleProject } });
