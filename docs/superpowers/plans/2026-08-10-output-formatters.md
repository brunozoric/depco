...
import type {
  ...
  IOutputFormatterFactoryInput
} from "./abstractions/OutputFormatterFactory.js";
import { TableFormatter } from "./TableFormatter.js";
import { JsonFormatter } from "./JsonFormatter.js";
  ...
  public format(output: IScanOutput): string {
    ...
    for (const finding of output.findings.license) {
      ...
      results.push({
        ...
        ruleIndex,
        message: {
          text: `Package ${finding.packageName}@${finding.version} uses ${finding.license} license (${finding.riskTier} risk)`
        },
    ...
    for (const finding of output.findings.vulnerability) {
      ...
      results.push({
        ...
        ruleIndex,
        message: {
          text: `Package ${finding.packageName}@${finding.installedVersion} has ${finding.severity} vulnerability ${finding.cveId ?? finding.dedupKey}`
        },
        locations: [{ physicalLocation: { artifactLocation: { uri: "package.json" } } }],
        properties: {
...
class RenderOutputStepImpl implements Abstraction.Interface {
  ...
  public async execute(context: IStepContext): Promise<IStepResult> {
    ...
    const packages =
      (context.results.get("packages") as Array<{ name: string; version: string }>) ?? [];
